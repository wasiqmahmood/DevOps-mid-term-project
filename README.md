...
SLACK_WEBHOOK=$(source /etc/profile.d/backup_env.sh; echo $SLACK_WEBHOOK)
...
process_log() {
  ...
  if [[ "$current_size" -gt "$last_size" ]]; then
    new_content=$(tail -c +$((last_size + 1)) "$log")
    ...
    for term in "${KEYWORDS[@]}"; do
      if grep -iq -m1 "$term" <<< "$new_content"; then
        ...
        [ -n "$SLACK_WEBHOOK" ] && send_slack_alert "$alert_message" "$log_excerpt"
      fi
    done
...
/var/log/backup_*.log /var/log/monitor.log {
    weekly
    rotate 8
    compress
    delaycompress
...
| **Security**      | Credential isolation, SELinux/AppArmor support |
...
All scripts are fully compatible with each other and include the improvements we discussed earlier (error handling, space validation, Slack formatting, etc.). The documentation is ready to be saved directly as a PDF.
